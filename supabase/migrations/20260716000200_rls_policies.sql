...
-- text, so it fails with "operator does not exist: uuid = text".
--
-- We use the first form but in the `public` schema, not `auth`: Supabase revokes
-- CREATE on the auth schema, so `create function auth.org_id()` fails with
-- "permission denied for schema auth". Calling auth.uid() is still fine — that
-- is reading, not creating. The cast lives in one audited place either way.
--
...
create policy members_insert on org_members
  for insert
  to authenticated
  with check (org_id = public.org_id() and public.user_role() = 'owner');

create policy members_update on org_members
  for update
  to authenticated
  using (org_id = public.org_id() and public.user_role() = 'owner')
  with check (org_id = public.org_id() and public.user_role() = 'owner');
...
create policy devices_read on devices
  ...
  using (
    ...
    and (user_id = (select auth.uid()) or public.user_role() = 'owner')
  );
...
create policy devices_insert on devices
  for insert
  to authenticated
  with check (org_id = public.org_id() and user_id = (select auth.uid()));
...
create policy audit_read on audit_logs
  for select
  to authenticated
  using (org_id = public.org_id() and public.user_role() in ('owner', 'accountant'));
...
create policy owner_updates_organization on organizations
  for update
  to authenticated
  using (id = public.org_id() and public.user_role() = 'owner')
  with check (id = public.org_id() and public.user_role() = 'owner');
...
-- The profile row is created by the app right after OTP verification, because
-- Supabase forbids the usual `on auth.users` trigger (no CREATE on the auth
-- schema). `with check` pins the row to the caller: you may create your own
