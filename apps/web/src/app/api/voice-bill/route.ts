...
interface CatalogItem { name?: string; price?: number; gst?: number }
...
export async function POST(req: Request): Promise<Response> {
  ...
  const { transcript, products } = (body ?? {}) as { transcript?: unknown; products?: unknown };
  ...
  const system =
    ...
    (catalog ? `CATALOGUE (name | price | GST):\n${catalog}` : "No catalogue provided.");
  ...
  try {
    const r = await fetch(url, {
      ...
      body: JSON.stringify({
        contents: [{ role: "user", parts: [{ text: transcript.slice(0, 1500) }] }],
    ...
    if (!parsed) return NextResponse.json({ error: "Could not understand the order." }, { status: 422 });
